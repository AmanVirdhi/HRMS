...
export class EmployeeDashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  //         this.filteredPersDetails = [...this.categoryGroup];
  ...
  search() {
    if (this.searchQuery.trim()) {
      this.filteredPersDetails = this.categoryGroup.filter(item =>
        item.fullName.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    }
    else {
      this.filteredPersDetails = [...this.categoryGroup];
      if (this.filteredPersDetails.length === 0) {
    ...
    this.EmployeeServiceService.getListEmployee().subscribe(
      (res) => {
        ...
        if (empRegId == '6') {
          ...
          this.filteredPersDetails = this.categoryGroup.filter(item => item.empPersDtlId === parseInt(empRegId));
        }

        // if (empRegId ) {
        //   this.filteredPersDetails = this.categoryGroup.filter(item => item.empPersDtlId === parseInt(empRegId));
        // } else {
        //   this.filteredPersDetails = [];
        // }
        this.showOptions = this.filteredPersDetails.map(() => false);
        this.loading = false;
        this.observeLastCard();
      },
      (error) => {
  ...
  //#endregion
  //#region added by Amandeep on 30-07-2024 for department., emp id from Employement api using employement
  employment: any;
